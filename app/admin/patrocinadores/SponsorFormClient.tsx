...
type Props = {
  upsertSponsor: (formData: FormData) => Promise<void>;
};
...
export default function SponsorFormClient({ upsertSponsor }: Props) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // reset correto (evita o erro "reading 'reset'")
    ...
    <form onSubmit={onSubmit} className="mt-4 space-y-3">
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <div>
          ...
          <div className="mt-1 aspect-[16/9] w-full overflow-hidden rounded-2xl border border-line/60 bg-black/30">
            {preview ? (
              <img src={preview} alt="preview" className="h-full w-full object-contain p-3" />
            ) : (
              <div className="grid h-full w-full place-items-center text-xs text-muted">Sem logo</div>
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mt-2 text-[11px] text-muted">
            Se não escolher arquivo, cole a URL da logo aqui.
          </div>
        </div>
      </div>
